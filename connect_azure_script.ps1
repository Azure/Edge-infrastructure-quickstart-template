...
$subscription = az account show --subscription $subscriptionId

if (-not $subscription) {
    throw "Subscription $subscriptionId not found."
}
$subTenant = echo $subscription | ConvertFrom-Json | Select-Object -ExpandProperty tenantId

$account = az account show
$accountTenant = echo $account | ConvertFrom-Json | Select-Object -ExpandProperty tenantId

if ($subTenant -ne $accountTenant) {
    echo "Switching to subscription tenant $subTenant..."
    az login --tenant $subTenant
}
...
$servicePrincipalCreation = Read-Host "Do you want to create a service principal for provisioning resources to Azure from this repository? (Y/n)"
if (!($servicePrincipalCreation -eq "n")) {

    # Create service principal if not exists
    $sp = $null

    $servicePrincipalName = Read-Host "Enter the name of the service principal."
...
"@
    ...
    echo "Granting permission Contributor/Key vault secrets officer/User Access Administrator/Storage Blob Data Owner to the service principal $spid..."
    ...
    az role assignment create --role "Storage Blob Data Owner" --assignee $spid --scope "/subscriptions/$subscriptionId"

    $reuseForArb = Read-Host "Do you want to reuse the created service principal for Arc Resource Bridge? (y/N)"
    if ($reuseForArb -eq "Y") {
        echo "Service principal secret is required for Arc Resource Bridge..."
        $confirmResetSecret = Read-Host "Do you want to reset the secret for the service principal? (y/N)"
        if ($confirmResetSecret -eq "Y") {
            $servicePrincipalSecret = (az ad app credential reset --id $spid | ConvertFrom-Json | Select-Object -ExpandProperty "password").Trim('"')
        } else {
            $servicePrincipalSecret = Read-Host "Enter the secret for the service principal." -AsSecureString
...
if (-not $spid) {
    $spid = Read-Host "Enter the service principal client id (app id) for provisioning resources to Azure."
}
...
$domainAdminPassword = Read-Host "Enter the admin password of domain controller. (Leave it empty if you prepare AD yourself)" -AsSecureString
gh secret set domainAdminPassword -b ([System.Net.NetworkCredential]::new("", $domainAdminPassword).Password)
$localAdminUser = Read-Host "Enter the local admin user name of HCI hosts."
gh secret set localAdminUser -b $localAdminUser
$localAdminPassword = Read-Host "Enter the local admin password of HCI hosts." -AsSecureString
gh secret set localAdminPassword -b ([System.Net.NetworkCredential]::new("", $localAdminPassword).Password)
$deploymentUserPassword = Read-Host "Enter the password for the deployment user." -AsSecureString
gh secret set deploymentUserPassword -b ([System.Net.NetworkCredential]::new("", $deploymentUserPassword).Password)

if ($reuseForArb -eq "Y") {
    gh secret set servicePrincipalId -b $spid
    gh secret set servicePrincipalSecret -b $servicePrincipalSecret
} else {
    $arbServicePrincipalId = Read-Host "Enter the service principal id for Arc Resource Bridge."
    gh secret set servicePrincipalId -b $arbServicePrincipalId
    $arbServicePrincipalSecret = Read-Host "Enter the service principal secret for Arc Resource Bridge." -AsSecureString
    gh secret set servicePrincipalSecret -b ([System.Net.NetworkCredential]::new("", $arbServicePrincipalSecret).Password)
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAjOLOtx4gR/T14
# APi3ze+3iaPUtvZs0yswHbKCqFVnjaCCDXYwggX0MIID3KADAgECAhMzAAADrzBA
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIGbHQWWsi8f/aTd7XGoQudE3
# sS5x1LtE7WbscPImTKa1MEIGCisGAQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8A
# cwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEB
# BQAEggEAiYfiaG0H9JXBdy5b1aLHi3jRXx5HRKpE6Vssgrm6azeysQ4nPQMRgFBY
# K9j3lo9m2FWHaPaFLFFtLe5uuOK9xpqDm9hhKmlzBZ4axFieyabtKZVRHWlEUUHv
# oOVhEsLFfG44IZjp/PoSQZwk/wjMQEzO2f/vFjPZ/xDTTcUnZe4tZjezHuc/EQXH
# /VlVztfS8UMedFWAycrauXZhNum1crLQN3AcsHU8ITwLyyzM/oDMfcJFvP598y7v
# eAdgTBc56Vw9fcQm2gmEPAta9CHjyVRjdk4c+al3fLJA5xssdyYRwwp9JpeqHESQ
# ZQZDQz5hd+cSnI6KbPODl6rI93AV96GCF5cwgheTBgorBgEEAYI3AwMBMYIXgzCC
# F38GCSqGSIb3DQEHAqCCF3AwghdsAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFSBgsq
# hkiG9w0BCRABBKCCAUEEggE9MIIBOQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFl
# AwQCAQUABCDSswL2FTUUf2LC4bnrsKdv+42wDVwnvdsfYvyAaYzM9wIGZpVuVGfB
# GBMyMDI0MDgwOTAzMDQ0MC41MTVaMASAAgH0oIHRpIHOMIHLMQswCQYDVQQGEwJV
...
# IFBDQSAyMDEwMA0GCSqGSIb3DQEBCwUAAgUA6l+O1DAiGA8yMDI0MDgwODE4Mzcw
# OFoYDzIwMjQwODA5MTgzNzA4WjB3MD0GCisGAQQBhFkKBAExLzAtMAoCBQDqX47U
# AgEAMAoCAQACAhnrAgH/MAcCAQACAhMlMAoCBQDqYOBUAgEAMDYGCisGAQQBhFkK
# BAIxKDAmMAwGCisGAQQBhFkKAwKgCjAIAgEAAgMHoSChCjAIAgEAAgMBhqAwDQYJ
# KoZIhvcNAQELBQADggEBACK7relzNDxlNP4/AJ2sontnSAleojh790PpSdYImSR6
# oAFc1W3CWg2y2tCR1DnK68UZXHMxJwEPRH6K5Sp+yEgOIXvsl1kmBRSVY+Z2U5dT
# pKloFSgj0hL6rS8iXIbP/Rt3C6dY8BGEZRyQuO+rHeGND8L78ID/ojIibzHQOBVS
# SgE4kkCOhpCuzIieXUhWRv3rxxKBEDYdDb0jw+npwYyo/+gVyMZvOcVsr8v8Brzz
# Si6VdL/K7NrHxySbyqQPvCaejSDVEU9sXdD/CqSjx3YrMfUuBtWfUWWH4d5OTJV3
# PpidilFvOQHP5IMyU8XgrIO8cr19mNrw+3yNX05xLTUxggQNMIIECQIBATCBkzB8
...
# MC8GCSqGSIb3DQEJBDEiBCAS5hchGOBiaoIc5SpTiGVIY6oaRdMh6/pkVHfHiuOb
...
# IDIwMTACEzMAAAHwKnwdWTvmH60AAQAAAfAwIgQgpgU4bbITa+m411psknX9+5zm
# 9wJWFSgUpaWkZG/EkVAwDQYJKoZIhvcNAQELBQAEggIAjOUNVgL8+VD+T64it+vG
# CdoHXx6xrEX4GkyFRqm80Rrp1NbEGJAQoQxMUdzrIunNQM3d0mQPqtUZhoM3yc76
# fEaBkIoLHy1MhAgpSUAww/GEYQFs8oNsrWV98g+U3uetinU8bOW3YYz0+iLgykW6
# TOuwEwsHbqipgpvb5wrdZ1hdCoo2a7JYug6UDSdKfkOd4PELFblq+m+g+GLouYVg
# O78Y2Ydju0BjyMaqrMhKx9xjFYy215Sgay9MkmXjkeLyNZfGWWMceMnZbHCmbEbR
# FW2h8UwqJrVutQOzMEsylfEK4bLLJR1XquBP5SRC6WCmat4Xn15wIKR4TjRbFSyd
# vjtAVPW0RDcZLE4wjnJ/tj+/T2qEGf961ZNiT7g2IPIY+DeHXiPBmyX9bOVe1Los
# nTAfD2kRWFlYpILKcYD+RmYrJrBX2A9CmFDoMc6ITebz1OZPAHwTk+tAnaebEs+v
# vVUhgSw8a+k9FBCZygR5mwKTE/Gm9308pImrADoG+CxYWKqL2a4glbOR7qbgm4E3
# rYN8gi5FhqcyevCImJ1OHMGAizk+MUDK37tGFYqU/997xW5SNmTco/+ofkjpTBYN
# Q1gdlSyC8sBIgOl1gMSRbtqUpn8eOEiScrkR3qpMhCn5HKYSZTj5GuhewoxvM/Zh
# 82kfb2XLPdqqwsXNhuQdRMM=
