 module "base" {
   ...
   startingAddress = "172.25.85.20"
   endingAddress   = "172.25.85.29"
   defaultGateway  = "172.25.85.1"
   ...
   servers = [
     {
       name        = "CA-Node1",
       ipv4Address = "172.25.85.11"
     },
     {
       name        = "CA-Node2",
       ipv4Address = "172.25.85.12"
     }
   ]
  ...
  #  Region HCI logical network parameters
   lnet-startingAddress = "172.25.85.40"
   lnet-endingAddress   = "172.25.85.55"   #This IP range should not overlap with HCI infra IP range.
   lnet-addressPrefix   = "172.25.85.0/26"   #E.g., 192.168.1.0/24
   lnet-defaultGateway  = "172.25.85.1" # Default gateway can be same as HCI infra default gateway.
   lnet-dnsServers      = ["10.254.0.196", "10.254.0.197"]   # DNS servers can be same as HCI infra DNS servers.

#   # Region AKS Arc parameters
   aksArc-controlPlaneIp   = "172.25.85.56"      # An IP address in the logical network IP range.
